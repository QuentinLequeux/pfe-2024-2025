                    ...
                    {animals.data.length > 0 ? (
                        <div className={'mt-8 flex flex-wrap justify-center gap-8'}>
                            {animals.data.map((animal) => (
                                <Link title={`Vers la fiche de ${animal.name}`} href={route('animals.show', { animal: animal.slug})} key={animal.id} className={`hover:scale-105 h-auto w-[250px] rounded-lg bg-[#fff] dark:bg-[#1c1e21] shadow-lg ${animal.adoption_status === `Adopté` ? "pointer-events-none opacity-50" : ""}`}>
                                    <div className={'relative'}>
                                        ...
                                        <img className={'h-auto rounded-t-lg'} src={animal.photo_url?.medium ?? 'https://fastly.picsum.photos/id/237/250/190.jpg?hmac=Ytps3oz1RzMVeuF4dclkzZL2SmeBKE_2-sWjFcjVRRk'} alt={`Photo de ${animal.name}`} width={250} height={190} />
