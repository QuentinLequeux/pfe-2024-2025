                    ...
                    {animals.data.length > 0 ? (
                        <div className={'mt-8 flex flex-wrap justify-center gap-8'}>
                            {animals.data.map((animal) => (
                                <Link title={`Vers la fiche de ${animal.name}`} href={route('animals.show', { animal: animal.slug})} key={animal.id} className={`h-auto w-[250px] rounded-lg bg-[#fff] dark:bg-[#1c1e21] shadow-lg ${animal.adoption_status === `Adopté` ? "pointer-events-none opacity-50" : ""}`}>
                                    <div className={'relative'}>
